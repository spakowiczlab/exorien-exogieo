---
title: "EO v LO in ORIEN consortium data"
author: "Rebecca Hoyd"
date: "5/10/2023"
...
```{r}
clin <- read.csv("../data/clinical_ORIEN_curated.csv")
...
loadCounts <- function(){
  ...
  goodsamps <- intersect(exo.ra$sample, clin$RNASeq)
...
exo.counts <- lapply(taxalevels, function(x) exoRAtowidecounts(combined.data, x))
names(exo.counts) <- taxalevels

clindat.binage <- clin %>%
  dplyr::select(RNASeq, agestat) 
```

```{r}
res.list <- lapply(taxalevels, function(x) DESeq.lev(exo.counts[[x]], clindat.binage, x))
...
res.df %>%
  ...
  write.csv("../data/deseq2_all-cohort_ORIEN-data.csv", row.names = F)
```

Save intermediate object for quick figure adjustments.
```{r}
res.df <- read.csv("../data/deseq2_all-cohort_ORIEN-data.csv")
```
...
ggsave(volplot, file = "../figures/volcano_ORIEN-data_deseq2-allsamps.png",
...
lda.vis.list <- lapply(c(5,10,15), function(x) generate.lda.vis(x))
names(lda.vis.list) <- c("5t", "10t", "15t")
lapply(names(lda.vis.list), function (x) 
  ggsave(plot = lda.vis.list[[x]], 
         filename = paste0("../figures/LDA_ORIEN-data_deseq2_", x, ".png"),
